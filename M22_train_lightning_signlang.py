...
class MulticlassFDR(tm.Metric):
    """
    Реализация метрики FDR
    """

    def __init__(self, num_classes: int):
        super().__init__()
        self.num_classes = num_classes
        self.add_state("total_fdr", default=torch.tensor(0.0), dist_reduce_fx="sum")
        self.add_state("valid_cls", default=torch.tensor(0), dist_reduce_fx="sum")
    ...
    def update(self, preds: torch.Tensor, target: torch.Tensor):
        ...
        for class_idx in range(self.num_classes):
            # Для каждого класса вычисляем TP и FP
            tp = ((preds == class_idx) & (target == class_idx)).sum().float()
            fp = ((preds == class_idx) & (target != class_idx)).sum().float()

            denominator = fp + tp
            if denominator > 0:
                class_fdr = fp / denominator
                batch_fdr += class_fdr
                batch_valid += 1
            # Если denominator == 0, просто пропускаем этот класс
            # (не добавляем в batch_valid и не увеличиваем batch_fdr)

        if batch_valid > 0:
            self.total_fdr += batch_fdr
            self.valid_cls += batch_valid

    def compute(self) -> torch.Tensor:
        return self.total_fdr / self.valid_cls if self.valid_cls > 0 else torch.tensor(0.0)
...
class CFG:
    ...
    experiment_name: str = "Lightning CNN + Torchmetrics"
...
class LitSignLang(pl.LightningModule):
    ...
    def __init__(self, lr: float = 1e-3, n_classes: int = 25):
        ...
        # Метрики
        self.train_metric_acc = tm.Accuracy(task="multiclass", num_classes=n_classes)
        self.valid_metric_acc = tm.Accuracy(task="multiclass", num_classes=n_classes)
        self.train_metric_fbs = tm.FBetaScore(task="multiclass", num_classes=n_classes,
                                              average="weighted", beta=0.5)
        self.valid_metric_fbs = tm.FBetaScore(task="multiclass", num_classes=n_classes,
                                              average="weighted", beta=0.5)
        self.train_metric_auc = tm.AUROC(task="multiclass", num_classes=n_classes).cpu()
        self.valid_metric_auc = tm.AUROC(task="multiclass", num_classes=n_classes).cpu()
        self.train_metric_fdr = MulticlassFDR(num_classes=n_classes)
        self.valid_metric_fdr = MulticlassFDR(num_classes=n_classes)
    ...
    def basic_step(self, batch, batch_idx, step: str):
        ...
        probs = torch.softmax(logits, dim=1).cpu()  # на ГПУ не работает

        with warnings.catch_warnings():
            warnings.simplefilter("ignore")  # Подавляем предупреждения при обновлении метрик
            # Обновляем метрики
            acc_metric = getattr(self, f'{step}_metric_acc')(preds, y)
            fbs_metric = getattr(self, f'{step}_metric_fbs')(preds, y)
            fdr_metric = getattr(self, f'{step}_metric_fdr')(preds, y)
            auc_metric = getattr(self, f'{step}_metric_auc')(probs, y.cpu())

        # Логирование в Lightning
        metrics_dict = {f"{step}_fbeta": fbs_metric,
                        f"{step}_loss": loss,
                        f"{step}_acc": acc_metric,
                        f"{step}_auc": auc_metric,
                        f"{step}_fdr": fdr_metric,
                        }

        self.log_dict(metrics_dict, prog_bar=True)

        # Логирование в ClearML
        logger_clearml.report_scalar(f"{step}_fbeta", "epoch", fbs_metric, self.current_epoch)
        logger_clearml.report_scalar(f"{step}_loss", "epoch", loss.item(), self.current_epoch)
        logger_clearml.report_scalar(f"{step}_acc", "epoch", acc_metric, self.current_epoch)
        logger_clearml.report_scalar(f"{step}_auc", "epoch", auc_metric, self.current_epoch)
        logger_clearml.report_scalar(f"{step}_fdr", "epoch", fdr_metric, self.current_epoch)
...
def run_training(args, datamodule: SignLanguageDataModule):
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        filename="best_model",
        save_top_k=1,
        monitor="valid_auc",  # метрика для раннего стопа
    ...
    on_exception_callback = OnExceptionCheckpoint(dirpath=MODEL_DIR,
    ...
    # Accuracy curves
    plt.subplot(1, 2, 2)
    plt.plot(metrics["train_acc"].dropna(), label="train_acc")
